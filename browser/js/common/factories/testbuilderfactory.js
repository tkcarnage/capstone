...
app.factory('TestBuilderFactory', function($http, AuthService){
	...
	testobj.create = function(obj){
		...
		return $http.post('http://localhost:1337/api/tests/', clonedObj)
		.then(response =>  {
    ...
    testobj.edit = function(obj){
        ...
        clonedObj.body.data = JSON.stringify(clonedObj.body.data);
        return $http.put('http://localhost:1337/api/tests/' + clonedObj._id, clonedObj)
        .then(response => response.data);
    };
    testobj.delete = function(obj){
        return $http.delete('http://localhost:1337/api/tests/' + obj._id)
    };
